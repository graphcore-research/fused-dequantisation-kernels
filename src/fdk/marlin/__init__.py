...
def _load_extension() -> object:
    ...
    return torch.utils.cpp_extension.load(
        name="marlin_cuda",
        sources=list([dir_ / p for p in ["marlin_cuda.cpp", "marlin_cuda_kernel.cu"]]),
        verbose=True,
    )


marlin_cuda = _load_extension()


def mul(A, B, C, s, centroids, workspace, thread_k=-1, thread_n=-1, sms=-1, max_par=16):
    ...
    @centroids: `torch.half` centroids of shape `(16,)` or `None` for linear quantization
    ...
    """
    marlin_cuda.mul(A, B, C, s, centroids, workspace, thread_k, thread_n, sms, max_par)
...
def _get_perms(is_linear):
    ...
    perm = np.array(perm)
    if is_linear:
        # Original Marlin nibble ordering
        interleave = np.array([0, 2, 4, 6, 1, 3, 5, 7])
    else:
        # When using a 2D lookup table, keep consecutive nibbles together
        interleave = np.array([0, 1, 4, 5, 2, 3, 6, 7])
...
class Layer(nn.Module):
    ...
    def __init__(self, infeatures, outfeatures, groupsize=-1, is_linear=False):
        ...
        @groupsize: quantization groupsize (must be -1, 128 or 64)
        """
        super().__init__()
        if groupsize not in [-1, 128, 64]:
            raise ValueError("Only groupsize -1, 128 and 64 are supported.")
        ...
        if is_linear:
            ...
            self.register_buffer("centroids", torch.randn((256, 2), dtype=torch.half))
    ...
    def pack(self, linear, scales):
        ...
        """
        _perm, _scale_perm, _scale_perm_single = _get_perms(is_linear=self.is_linear)
    ...
    for attr in dir(module):
        ...
        if isinstance(tmp, nn.Linear) and name_filter(name1):
            setattr(
                module,
                attr,
                Layer(tmp.in_features, tmp.out_features, groupsize=groupsize),
            )
