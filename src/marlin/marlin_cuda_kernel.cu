...
__device__ inline FragB dequant_linear(int q) {
...
__device__ __forceinline__ FragB dequant_nonlinear(int q, const half2* __restrict__ centroids) {
  // Elements 0-1 (bits 0-7)
  int c01 = q & 0xFF;
  // Elements 2-3 (bits 16-23)
  int c23 = (q >> 16) & 0xFF;

  // Use read-only loads so the hardware can broadcast
  FragB frag_b;
  frag_b[0] = __ldg(centroids + c01);
  frag_b[1] = __ldg(centroids + c23);
  return frag_b;
}
...
__device__ __forceinline__ void scale(FragB& frag_b, FragS& frag_s, int i) {
  // frag_s is Vec<half2, 1>; treat it as an array of half
  half2 s = __half2half2(reinterpret_cast<const __half*>(&frag_s)[i]);
...
template <
  ...
  const bool is_linear = false // switch between linear (ignore centroids) and non-linear (use centroids) formats
>
__global__ void Marlin(
  ...
  const half2* __restrict__ centroids, // centroids of shape 256 (byte lookup -> half2)
  ...
  const half2* __restrict__ centroids_ptr = centroids;
  ...
  static_assert(group_blocks == -1 || (group_blocks >= thread_k_blocks && (group_blocks % thread_k_blocks) == 0),
    "GROUP_BLOCKS must be -1 or a multiple of THREAD_K_BLOCKS");
  if (group_blocks != -1)
  ...
  auto matmul = [&] (int k) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      FragB frag_b0 = is_linear ? dequant_linear(b_quant) : dequant_nonlinear(b_quant, centroids_ptr);
      // If there are no groups, we can just scale the final output once and can avoid doing so for each weight.
      if (group_blocks != -1)
        scale(frag_b0, frag_s[k % 2][j], 0);
      FragB frag_b1 = is_linear ? dequant_linear(b_quant_shift) : dequant_nonlinear(b_quant_shift, centroids_ptr);
      if (group_blocks != -1)
...
#define CALL_IF(THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS, IS_LINEAR) \
  else if ( \
    thread_m_blocks == THREAD_M_BLOCKS && thread_n_blocks == THREAD_N_BLOCKS && thread_k_blocks == THREAD_K_BLOCKS && \
    group_blocks == GROUP_BLOCKS && is_linear == IS_LINEAR \
  ) { \
    cudaFuncSetAttribute( \
      Marlin<THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS, IS_LINEAR>, \
      cudaFuncAttributeMaxDynamicSharedMemorySize, \
      SHARED_MEM \
    ); \
    Marlin< \
      THREADS, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, STAGES, GROUP_BLOCKS, IS_LINEAR \
    ><<<blocks, THREADS, SHARED_MEM, stream>>>( \
      A_ptr, B_ptr, C_ptr, s_ptr, centroids_ptr, \
...
int marlin_cuda(
  ...
  if (thread_k == -1 || thread_n == -1) {
    if (prob_m <= 16 && (groupsize == -1 || groupsize >= 128)) {
  ...
  const half2* centroids_ptr = (const half2*) centroids;
  bool is_linear = (centroids == nullptr);
  ...
  for (int i = 0; i < tot_m_blocks; i += 4) {
    ...
    if (false) {}
    // THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, GROUP_BLOCKS, IS_LINEAR
    // is_linear=true
    CALL_IF(1,  8,  8, -1, true)
    CALL_IF(1,  8,  8,  8, true)
    // CALL_IF(1,  8,  8,  4, true)  // illegal (group_blocks < thread_k_blocks)

    CALL_IF(1, 16,  4, -1, true)
    CALL_IF(1, 16,  4,  8, true)
    CALL_IF(1, 16,  4,  4, true)

    CALL_IF(2, 16,  4, -1, true)
    CALL_IF(2, 16,  4,  8, true)
    CALL_IF(2, 16,  4,  4, true)

    CALL_IF(3, 16,  4, -1, true)
    CALL_IF(3, 16,  4,  8, true)
    CALL_IF(3, 16,  4,  4, true)

    CALL_IF(4, 16,  4, -1, true)
    CALL_IF(4, 16,  4,  8, true)
    CALL_IF(4, 16,  4,  4, true)

    // is_linear=false
    CALL_IF(1,  8,  8, -1, false)
    CALL_IF(1,  8,  8,  8, false)
    // CALL_IF(1,  8,  8,  4, false)  // illegal (group_blocks < thread_k_blocks)

    CALL_IF(1, 16,  4, -1, false)
    CALL_IF(1, 16,  4,  8, false)
    CALL_IF(1, 16,  4,  4, false)

    CALL_IF(2, 16,  4, -1, false)
    CALL_IF(2, 16,  4,  8, false)
    CALL_IF(2, 16,  4,  4, false)

    CALL_IF(3, 16,  4, -1, false)
    CALL_IF(3, 16,  4,  8, false)
    CALL_IF(3, 16,  4,  4, false)

    CALL_IF(4, 16,  4, -1, false)
    CALL_IF(4, 16,  4,  8, false)
    CALL_IF(4, 16,  4,  4, false)
    else
      ret = ERR_KERN_SHAPE;
